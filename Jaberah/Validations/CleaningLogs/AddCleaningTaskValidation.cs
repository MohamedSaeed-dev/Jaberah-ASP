...
namespace Jaberah.Validations.CleaningLogs
{
    ...
    public class AddCleaningTaskAttribute : ActionFilterAttribute
    {
        public override void OnActionExecuting(ActionExecutingContext context)
        {
            List<ValidationModel> validationContent = new();

            if (context.ActionArguments.TryGetValue("model", out var argument) && argument is AddCleaningTaskDTO dto)
            {
                if (string.IsNullOrWhiteSpace(dto.NameAr))
                {
                    ...
                    {
                        Key = "اسم المهمة",
                        Message = "اسم المهمة اجباري"
                    });
                }
                else if (!dto.NameAr.ContainsArabic())
                {
                    validationContent.Add(new ValidationModel
                    {
                        Key = "اسم المهمة",
